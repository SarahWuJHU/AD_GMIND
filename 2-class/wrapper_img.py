...
def helper_test(net, x_g_j, x_i_j, y_j, D, typ, hyp, e, metric, T):
    ...
    with torch.no_grad():            
        ...
        metric[typ]['Sensitivity'][e] = metrics.recall_score(y_label, y_predict, pos_label=1)
        metric[typ]['Specificity'][e] = metrics.recall_score(y_label, y_predict, pos_label=0)
...
def helper_plot(M, e, f, st):
    for i in M:
        if i!='class_pred' and i!='class_true':
            ...
            plt.savefig('data_'+str(id)+'/model'+str(f)+'/'+st+'_'+i+'.pdf')
            plt.close()  
    return
...
def save_results(net, metric_losses, check_losses, train_losses, f, prob_d):
    ...
    sio.savemat('data_'+str(id)+'/setting/model'+str(f)+'/vis'+'.mat',vis)   
...
for id in range(100):
    id_cv = id//10 + 1
    id_fold = id % 10 + 1
    pwd = '/home/sarah/Desktop/GMIND/2_class/'
    cv_folder = 'cross_val/cv' + str(id_cv) + '/'
    I_train = cpu_t(loadmat(pwd + cv_folder + '/fold' + str(id_fold) + '.mat')['I_train'])
    G_train = cpu_t(loadmat(pwd + cv_folder + '/fold' + str(id_fold) + '.mat')['G_train'])
    Y_train = cpu_t(loadmat(pwd + cv_folder + '/fold' + str(id_fold) + '.mat')['Y_train_O'])

    ## Validation data
    I_val = cpu_t(loadmat(pwd + cv_folder + '/fold' + str(id_fold) + '.mat')['I_val'])
    G_val = cpu_t(loadmat(pwd + cv_folder + '/fold' + str(id_fold) + '.mat')['G_val'])
    Y_val = cpu_t(loadmat(pwd + cv_folder + '/fold' + str(id_fold) + '.mat')['Y_val_O'])

    # test data
    I_test = cpu_t(loadmat(pwd + cv_folder + '/fold' + str(id_fold) + '.mat')['I_test'])
    G_test = cpu_t(loadmat(pwd + cv_folder + '/fold' + str(id_fold) + '.mat')['G_test'])
    Y_test = cpu_t(loadmat(pwd + cv_folder + '/fold' + str(id_fold) + '.mat')['Y_test_O'])
    ...
    # merical range
    lambda_0 = [cpu_ts(0.001), cpu_ts(0.01),cpu_ts(0.5), cpu_ts(0.001)] # gene_recon, image_recon, class loss, sprasity
    ...
    if( not os.path.exists('data_'+str(id)) ):
        os.mkdir('data_'+str(id)) # Create cross validation folder f.

    if( not os.path.exists('data_'+str(id)+'/setting') ):
        os.mkdir('data_'+str(id)+'/setting') # Create cross validation folder f.
        
    
    for f in range(id, id+1):
        
        if( os.path.exists('data_'+str(id)+'/model'+str(f)) ):
            shutil.rmtree('data_'+str(id)+'/model'+str(f))
            
        print('fold - '+str(f))
        #shutil.rmtree('model'+str(f))
        os.mkdir('data_'+str(id)+'/model'+str(f)) # Create cross validation folder f.
        if  os.path.exists('data_'+str(id)+'/setting/model'+str(f)) :
            shutil.rmtree('data_'+str(id)+'/setting/model'+str(f))
            
        os.mkdir('data_'+str(id)+'/setting/model'+str(f)) # Create cross valida+ 0.1*loss2tion folder f.
        # Couple modules
        net =  cpu(joint_network(gene_layer, img_layer, l_dim))
        
        # init optimizer
        layers = [{'params':net.bias_ordinal,'lr':0.0001*learning_scale},{'params': net.encoder.parameters()}, {'params': net.decoder.parameters()}, \
            ...
            {'params': net.bias_i.parameters(), 'lr':0.0001*learning_scale}, {'params':net.bias_g.parameters(), 'lr':0.0004*learning_scale}]
        ...
        l = ['F1', 'Acc', 'Kappa', 'Sensitivity', 'Specificity']
        metric_losses['train']   = {}
        metric_losses['test']    = {}
        metric_losses['val']     = {}
        ...
        for e in range(num_epochs):
            ...
            metric_losses['train']['Sensitivity'][e] = metrics.recall_score(y_label, y_predict, pos_label=1)
            metric_losses['train']['Specificity'][e] = metrics.recall_score(y_label, y_predict, pos_label=0)
            ...
            if e%num_f == 0 :
                ...
                plt.savefig('data_'+str(id)+'/model'+str(f)+'/prob_pred_Gen.pdf')
                plt.close()
                
                gen_pred = cpu(net.prob[1]).data.numpy()
                plt.stem(list(range(np.shape(gen_pred)[0])),gen_pred)
                plt.savefig('data_'+str(id)+'/model'+str(f)+'/prob_pred_Nback.pdf')
                plt.close()
