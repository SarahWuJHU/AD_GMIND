...
def helper_test(net, x_g_j, x_i_j, y_j, D, typ, hyp, e, metric, T):
    ...
    with torch.no_grad():            
        ...
        metric[typ]['Sensitivity'][e] = metrics.recall_score(y_label, y_predict, average='macro')
        specs = []
        for i in range(1, 4):
            prec, recall,_, _ = metrics.precision_recall_fscore_support(y_label==i, y_predict==i, pos_label=True, average=None)
            specs.append(recall[0])
        metric[typ]['Specificity'][e] = sum(specs)/len(specs)
...
id_cv = id//10 + 1
id_fold = id % 10 + 1
pwd = '/home/sarah/Desktop/GMIND/3_class/'
cv_folder = 'cross_val/'
...
for f in range(id, id+1):
    ...
    for e in range(num_epochs):
        ...
        metric_losses['train']['Sensitivity'][e] = metrics.recall_score(y_label, y_predict, average='macro')
        specs = []
        for i in range(1, 4):
            prec, recall,_, _ = metrics.precision_recall_fscore_support(y_label==i, y_predict==i, pos_label=True, average=None)
            specs.append(recall[0])
        metric_losses['train']['Specificity'][e] = sum(specs)/len(specs)
